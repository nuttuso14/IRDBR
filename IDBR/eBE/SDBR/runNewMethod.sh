...
do  
    ...
    do  
			...
			#for deadline in 200 300 600 800 1000 1200 1800 
			for mean_money in 50 150 300 600 800 1000 1200 1800 
            do  
                # for i in $(seq 1 10)  
                # do  
                    # j=`expr $interArrivalT \* 10`  
                    echo "./main $arrRate $mean_service_t $max_drb_served_enb $wifi_mean_time_connected $wifi_mean_time_disconnected $deadline $algo $alpha $Ps $b $mean_file_size $idbr $pareto $mean_money"
                    ./main $arrRate $mean_service_t $max_drb_served_enb $wifi_mean_time_connected $wifi_mean_time_disconnected $deadline $algo $alpha $Ps $b $mean_file_size $idbr $pareto $mean_money
                    echo ""  
                    time sleep 0.6  
                # done  
                # echo  >> result.txt  
